...
% eval(['!3dcalc -a ERF_noise_abs_0.32-0.6s_Pat_01_13021km+tlrc -b ', PathMask_left, ' -exp a*b -prefix ',ROI, '_zscores'])
eval(['!3dExtrema -closure -volume -output ''Location_extrema', ROI,''' -mask_file ', PathMask_left, ' ERF_noise_abs_0.32-0.6s_', SubjectName,'+tlrc ']) 

end

function [LI_All_noise_abs]=collect_LI (SubjectPath, i, LI_All_noise_abs )
...
load(strcat(SubjectPath, filesep, 'LI_squared_noise_abs_Broca_SumMax_0.32_0.47ms.mat'))
LI_All(1,3)=LI_SumMax;
clear LI_SumMax

load(strcat(SubjectPath, filesep,  'LI_squared_noise_abs_Broca_SumMax_0.4_0.6ms.mat'))
LI_All(1,4)=LI_SumMax;
clear LI_SumMax


load(strcat(SubjectPath, filesep,   'LI_Max_squared_noise_abs_Wernicke_0.32_0.6ms.mat'))
LI_All(1,5)=LI_Max_sqared;
clear LI_Max

load(strcat(SubjectPath, filesep, 'LI_squared_noise_abs_Wernicke_SumMax_0.32_0.6ms.mat'))
LI_All(1,6)=LI_SumMax;
clear LI_SumMax

load(strcat(SubjectPath, filesep, 'LI_squared_noise_abs_Wernicke_SumMax_0.32_0.47ms.mat'))
LI_All(1,7)=LI_SumMax;
clear LI_SumMax

load(strcat(SubjectPath, filesep,  'LI_squared_noise_abs_Wernicke_SumMax_0.4_0.6ms.mat'))
LI_All(1,8)=LI_SumMax;
clear LI_SumMax

Path_LI_All = strcat(SubjectPath, filesep, 'LI_All_Maxima_noise_abs')
save(Path_LI_All, 'LI_All')

LI_All_noise_abs(i,:)=LI_All
