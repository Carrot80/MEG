...
function for_all (Folder, group)
   ...
   [fMRI_informed_MEG, fMRI_informed_MEG]=LI_informed(strcat(Folder, filesep, nameFolds{i,1}), nameFolds{i}, i, fMRI_informed_MEG, LI_fMRI_informed_MEG)
   
    end

    PathComparison=strcat('/home/kh/ShareWindows/Comparison_fMRI_MEG', filesep, 'Comparison_Overlap_MEG_fMRI_', group,'.mat')
...
Act_MEG_p05_perc=length(Act_MEG_p05)/length(Act_MEG_ROI)*100;

[ind_Act_fMRI_p05]=find(Act_fMRI_ROI>1.644853);
Act_fMRI_p05=Act_fMRI_ROI(ind_Act_fMRI_p05);
Act_fMRI_p05_perc=length(Act_fMRI_p05)/length(Act_fMRI_ROI)*100;
...
sum(ind_ismember);
overlap_fMRI_MEG=sum(ind_ismember)/length(ind_Act_MEG_p05)*100;
overlap_MEG_fMRI=sum(ind_ismember)/length(Act_fMRI_p05)*100;
overlap_both=sum(ind_ismember)/(length(ind_Act_fMRI_p05)+length(ind_Act_MEG_p05)-sum(ind_ismember))*100;
...
% hier weitermachen, zu klären: wie berechne ich LI?
