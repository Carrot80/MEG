...
function for_all_subjects  
      ...
      PatientFolder = 'D:\kirsten_thesis\data\controls\';
...
NewDir = (strcat('F:\Kirsten', filesep, SubjectName, filesep, 'SAM'));
if ~exist(NewDir, 'dir')
    
  mkdir(strcat('F:\Kirsten', filesep, SubjectName, filesep, 'SAM'));  
    
end

SAMPATH= (strcat('F:\Kirsten', filesep, SubjectName, filesep, 'SAM'));   

hs= strcat(Path.DataInput, filesep, 'hs_file');
FileBadTrials= strcat(Path.Preprocessing, filesep, 'BadTrials.mat');
CleanDataFile= strcat(Path.Preprocessing, filesep, 'CleanData.mat');
...
copyfile(FileBadTrials, SAMPATH)
...
if exist (CleanDataFile, 'file')
    copyfile(CleanDataFile, SAMPATH)
else
    disp(strcat('no CleanData file found in ', SubjectName))
end
